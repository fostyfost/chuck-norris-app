...
export const loader: LoaderFunction = async ({ request, params }) => {
  const prevPage = new URL(request.url).searchParams.get('prevPage') || '/jokes'
...
const JokeRoute = () => {
  const { prevPage, joke, isOwner } = useLoaderData<LoaderData>()

  return (
    <div>
      <Link to={prevPage} className='button mb'>
        Back
      </Link>
      <JokeDisplay name={joke.name} content={joke.content} isOwner={isOwner} />
    </div>
  )
}
